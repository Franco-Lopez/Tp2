...
public class Berretacoin {
    ...
    public Berretacoin(int n_usuarios) { //se construye el heap de usuarios con maxHeapDesdeSecuencia, que es O(P).
        ...
        for (int i = 0; i < usuarios.tamaño(); i++) {
            Usuario u = usuarios.verElemento(usuarios.handleDe(i));
            handlesUsuarios.set(u.id() - 1, usuarios.handleDe(i));
        }
    }

    public void agregarBloque(Transaccion[] transacciones) { //recorro las transacciones en O(n_b), actualizo montos en O(1) y actualizarPrioridad en O(log P) entonces: O(n_b × log P).
    ...
    public Transaccion[] txUltimoBloque() { //recorro la lista enlazada de nodos activos una vez
        ...
        if (ultimoBloque == null) {
            ...
            return ultimoBloque.obtenerTransacciones(); //recorre todos los nodos activos pero los invactivos son nodos null 
        }                                               //se ajustan los puntero y ese nodo ya no esta enlazado, no va a aparecer en actual.sig
    ...
    public void hackearTx() { //hago devolverMaximo() en O(log n_b)) + actualización de saldos y actualizarPrioridad() ×2 (O(log P)).
        Bloque ultimoBloque = blockchain.obtenerUltimo();
        if (ultimoBloque == null)
            return;
